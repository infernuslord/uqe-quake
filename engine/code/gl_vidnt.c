...
void CheckFramebufferExtension( void )
{
	...
	search = NULL;
	search = "GL_ARB_framebuffer_object";
	if (!search) return;
	...
	while( *tmp )
	{
		if (strncmp((const char*)tmp, search, strlen(search)) == 0)
		{
			Con_Printf("...Using GL_ARB_framebuffer_object\n");
	...
	if(!framebuffer_ext)
		Con_Printf("GL_ARB_framebuffer_object not found\n");
}
// jkrige - framebuffer object (bloom)
...
void VID_InitDIB (HINSTANCE hInstance)
{
    ...
    wc.lpszMenuName  = 0;
    wc.lpszClassName = "Quake"; // jkrige - was "WinQuake"
