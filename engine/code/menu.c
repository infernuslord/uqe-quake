...
void M_Options_Draw (void)
{
	...
	// jkrige - anisotropic filtering
	M_Print (16, 64, "          Texture Mode");
#ifdef GLQUAKE
	if (gl_texturemode.value == 0.0f)
		M_Print (220, 64, "Point");
	else if (gl_texturemode.value == 1.0f)
		M_Print (220, 64, "Bilinear");
	else if (gl_texturemode.value == 2.0f)
		M_Print (220, 64, "Trilinear");
	else
	{
		if(vid_scale2d.value == 0)
			M_Print (220, 64, va("Anisotropic %.0fx", pow(2.0f, gl_texturemode.value - 2.0f)));
		else
			M_Print (220, 64, va("AF %.0fx", pow(2.0f, gl_texturemode.value - 2.0f)));
	}
#else
	M_Print (220, 64, "Point");
#endif
	...
	M_Print (16, 136,	"            Mouse Look");
	M_DrawCheckbox (220, 136, cl_mlook.value /*in_mlook.state & 1*/); // jkrige - mlook cvar

	M_Print (16, 144,	"            Slope Look");
	M_DrawCheckbox (220, 144, cl_slook.value); // jkrige - slook cvar

	M_Print (16, 152,	"              Scale 2D");
	M_DrawCheckbox (220, 152, vid_scale2d.value); // jkrige - scale2d cvar
...
void M_Quit_Draw (void)
{
	...
	M_PrintWhite (16, 20,  "     http://www.corvinstein.com     ");
	// jkrige - new version
